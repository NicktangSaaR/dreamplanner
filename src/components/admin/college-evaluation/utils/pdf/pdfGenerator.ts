...
// Add font support
import { loadChineseFont, applyDocumentFont } from './fontUtils';
...
export const generateEvaluationPdf = async (evaluation: StudentEvaluation, universityType: UniversityType): Promise<jsPDF> => {
  ...
  try {
    // Load font support before rendering content
    ...
    doc.text("Error generating PDF. Please try again later.", 20, 20);
    return doc;
  }
};
